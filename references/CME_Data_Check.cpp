...
typedef struct FEED_DETAILS
{
        ...
        uint64_t                        DS_rcv_timestamp;
        uint64_t                        DS_snd_timestamp;
...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		int sz=0;
			...
			PacketCount++;
			FEED_DETAILS *ct = (FEED_DETAILS*)MulticastBuffer;
			std::cout << "sz:" << nRead << " payloadSize:" << payloadSize <<  std::endl;
			printf("----%s------\n",ct->SymbolCode);
			printf("%lu:%lu:%lu\n",ct->SrcTimestamp,ct->DS_rcv_timestamp,ct->DS_snd_timestamp);
			for(int k =0;k<FEED_LEVEL_DEPTH;k++){
				printf("%d-%d|%d-%d\n",ct->BuyQty[k],ct->BuyPrice[k],ct->SellPrice[k],ct->SellQty[k]);
			}
        	}
	}
	return 0;
