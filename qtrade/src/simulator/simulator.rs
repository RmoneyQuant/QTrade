...
pub enum ExecReport {
    ...
    /// `venue_order_id`: real MCX's `OrderID` (tag 37) -- assigned the
    /// instant an order is first accepted by the matching engine, absent
    /// from `Rejected` (never reached the book) and from `Canceled` (the
    /// id was already reported once, at `Resting`/`Filled` time).
    Filled { client_order_id: u64, venue_order_id: u64, price: Price, qty: Qty, kind: FillKind },
    Resting { client_order_id: u64, venue_order_id: u64, handle: OrderHandle, qty: Qty },
...
impl std::fmt::Display for ExecReport {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExecReport::Filled { client_order_id, venue_order_id, price, qty, kind } => {
                write!(f, "FILLED     id={client_order_id} venue_order_id={venue_order_id} price={} qty={} kind={kind:?}", price.0, qty.0)
            }
            ExecReport::Resting { client_order_id, venue_order_id, handle, qty } => {
                write!(
                    f,
                    "RESTING    id={client_order_id} venue_order_id={venue_order_id} side={:?} price={} qty={} priority_ts={}",
...
fn register_resting(
    ...
    resting_by_venue_order_id: &mut HashMap<u64, u64>,
    client_order_id: u64,
    sim_id: u64,
    venue_order_id: u64,
    instrument: InstrumentId,
    ...
    resting.insert(client_order_id, RestingSimOrder { instrument, side, price, sim_id, venue_order_id, remaining_qty: qty });
    resting_by_sim.insert(sim_id, client_order_id);
    resting_by_venue_order_id.insert(venue_order_id, client_order_id);
}
...
pub struct SimExchange {
    ...
    next_sim_seq: u64,
    /// Reverse index for `venue_order_id` (real MCX's `OrderID`, tag 37 --
    /// "generated by the MCX System; it remains constant"), the venue's
    /// own order identity, distinct from `client_order_id` (real MCX's
    /// `ClOrdID`, ours) and from `sim_id` (this file's own pre-existing,
    /// unrelated priority/queue-slot key, which *resets* on a
    /// price-changing modify -- `venue_order_id` deliberately does not,
    /// matching real `OrderID` semantics). Added 2026-09-02: real MCX
    /// Cancel/Replace requests require `OrderID`, not `ClOrdID` (§4.6/4.7)
    /// -- `ExecutionEngine` now sends this instead of `client_order_id` to
    /// every `cancel`/`modify` call, and this map is how that translates
    /// back to this struct's own `client_order_id`-keyed bookkeeping,
    /// same pattern `resting_by_sim` already uses for its own secondary
    /// key.
    resting_by_venue_order_id: HashMap<u64, u64>,
...
impl SimExchange {
    pub fn new(instruments: &[InstrumentId], otr_cfg: OtrConfig) -> Self {
        ...
        SimExchange {
            ...
            next_sim_seq: 0,
            resting_by_venue_order_id: HashMap::new(),
    ...
    /// Assigns a fresh `venue_order_id` -- called once per order the
    /// instant it's first accepted by the matching engine (never for a
    /// `Rejected` order, which never reached the book). No collision-
    /// avoidance offset needed the way `SIM_ID_BASE` gives `sim_id` one:
    /// unlike `sim_id`, this value never enters a book's own price-level
    /// slot structures (`add_sim`/`qty_ahead_of`), so it never shares a
    /// numeric space with real `priority_ts` values.
    fn next_venue_order_id(&mut self) -> u64 {
        let id = self.next_venue_order_seq;
            ...
            match c.owner {
                ...
                SlotOwner::Sim(id) => {
                    if let Some(&client_order_id) = self.resting_by_sim.get(&id) {
                        let (side, price, venue_order_id) = self
                            .resting
                            .get(&client_order_id)
                            .map(|r| (r.side, r.price, r.venue_order_id))
                            .unwrap_or((Side::Buy, Price(c.price_raw), 0));
        ...
        if let Some(&client_order_id) = self.resting_by_sim.get(&sim_id) {
            ...
            if done {
                if let Some(rec) = self.resting.remove(&client_order_id) {
                    self.resting_by_venue_order_id.remove(&rec.venue_order_id);
                }
        ...
        match req.order_type {
            OrderType::BookOrCancel(price) => {
                ...
                if crosses {
                    ...
                    register_resting(&mut self.resting, &mut self.resting_by_sim, &mut self.resting_by_venue_order_id, req.client_order_id, sim_id, venue_order_id, req.instrument, req.side, price, req.qty.0);
            ...
            OrderType::LimitDay(price) => {
                ...
                if filled > 0 {
                    reports.push(ExecReport::Filled { client_order_id: req.client_order_id, venue_order_id, price, qty: Qty(filled), kind: FillKind::Aggressive });
                ...
                if remaining > 0 {
                    ...
                    register_resting(&mut self.resting, &mut self.resting_by_sim, &mut self.resting_by_venue_order_id, req.client_order_id, sim_id, venue_order_id, req.instrument, req.side, price, remaining);
                    let qty_ahead = self.books[&req.instrument].qty_ahead_of(req.side, price.0, sim_id).unwrap_or(0);
            ...
            OrderType::MarketToLimit => {
                ...
                if consumed.is_empty() {
                    ...
                    let filled = self.settle_sweep(&consumed, &mut reports);
                    if filled > 0 {
                        reports.push(ExecReport::Filled { client_order_id: req.client_order_id, venue_order_id, price: Price(last_price), qty: Qty(filled), kind: FillKind::Aggressive });
                    }
                    let remaining = req.qty.0 - filled;
                    if remaining > 0 {
                        let sim_id = self.next_sim_id();
                        self.books.get_mut(&req.instrument).unwrap().add_sim(req.side, last_price, sim_id, remaining);
                        register_resting(&mut self.resting, &mut self.resting_by_sim, &mut self.resting_by_venue_order_id, req.client_order_id, sim_id, venue_order_id, req.instrument, req.side, Price(last_price), remaining);
    ...
    /// `venue_order_id` (real MCX's `OrderID`, tag 37) -- not
    /// `client_order_id`, matching real MCX's Cancel requirement
    /// (§4.6/4.7: "specify the OrderID... to identify the corresponding
    /// order to cancel"). `ExecutionEngine` is the one that translates a
    /// strategy's `client_order_id` into this before calling here.
    pub fn cancel(&mut self, venue_order_id: u64, now_ns: u64) -> Vec<ExecReport> {
        let mut reports = self.drain_due(now_ns as i64);
        if self.outbound_ns == 0 && self.inbound_ns == 0 {
            reports.extend(self.cancel_now(venue_order_id, now_ns));
        } else {
            self.in_transit.push((now_ns as i64 + self.outbound_ns as i64, PendingArrival::Cancel(venue_order_id)));
        }
        reports
    }

    fn cancel_now(&mut self, venue_order_id: u64, now_ns: u64) -> Vec<ExecReport> {
        let mut reports = Vec::new();
        let Some(&client_order_id) = self.resting_by_venue_order_id.get(&venue_order_id) else { return reports };
        if let Some(rec) = self.resting.remove(&client_order_id) {
    ...
    /// real orders above. `venue_order_id` itself never changes here,
    /// price-losing-priority or not -- matches real MCX's `OrderID`,
    /// which "remains constant" (unlike this file's own `sim_id`, an
    /// unrelated priority-queue key that does reset below).
    /// Public entry point (2026-08-27) -- same shape as `submit`/`cancel`.
    pub fn modify(&mut self, venue_order_id: u64, new_qty: Qty, new_price: Option<Price>, now_ns: u64) -> Vec<ExecReport> {
        let mut reports = self.drain_due(now_ns as i64);
        if self.outbound_ns == 0 && self.inbound_ns == 0 {
            reports.extend(self.modify_now(venue_order_id, new_qty, new_price, now_ns));
        } else {
            self.in_transit.push((now_ns as i64 + self.outbound_ns as i64, PendingArrival::Modify(venue_order_id, new_qty, new_price)));
        }
        reports
    }

    fn modify_now(&mut self, venue_order_id: u64, new_qty: Qty, new_price: Option<Price>, now_ns: u64) -> Vec<ExecReport> {
        let mut reports = Vec::new();
        let Some(&client_order_id) = self.resting_by_venue_order_id.get(&venue_order_id) else { return reports };
        ...
        if price_changed || qty_increased {
            ...
            self.resting_by_sim.remove(&rec.sim_id);
            register_resting(&mut self.resting, &mut self.resting_by_sim, &mut self.resting_by_venue_order_id, client_order_id, new_sim_id, rec.venue_order_id, rec.instrument, rec.side, target_price, new_qty.0);
        ...
        for (sim_id, qty) in fills {
            if let Some(&client_order_id) = self.resting_by_sim.get(&sim_id) {
                if let Some(r) = self.resting.get(&client_order_id).copied() {
                    ...
                    reports.push(ExecReport::Filled { client_order_id, venue_order_id: r.venue_order_id, price: r.price, qty: Qty(qty), kind: FillKind::Passive });
...
mod tests {
    ...
    fn qty_increase_loses_priority_even_at_same_price() {
        ...
        let ExecReport::Resting { venue_order_id, .. } = reports[0] else { panic!("expected Resting, got {:?}", reports[0]) };
        ex.apply_market_event(&add(DSide::Sell, 150, 5, 2), 0); // arrives after us, behind
        assert_eq!(ex.resting_qty_ahead(1), Some(20));
        ex.modify(venue_order_id, Qty(15), None, 0); // same price, qty INCREASE -- loses priority
        // now behind both real orders (20 + 5), even though price unchanged
        assert_eq!(ex.resting_qty_ahead(1), Some(25));
    }

    // ---- venue_order_id (real MCX's OrderID, tag 37) is modify-stable --
    // ---- the one behavior this identity split exists to guarantee,
    // ---- unlike sim_id, which deliberately does reset on the same modify ----

    #[test]
    fn venue_order_id_stays_constant_across_a_price_changing_modify() {
        let mut ex = exch();
        let reports = ex.submit(
            NewOrderRequest { client_order_id: 1, instrument: IID, side: Side::Buy, order_type: OrderType::LimitDay(Price(150)), qty: Qty(10) },
            0,
        );
        let ExecReport::Resting { venue_order_id: original_id, handle: original_handle, .. } = reports[0] else { panic!("expected Resting, got {:?}", reports[0]) };
        let reports = ex.modify(original_id, Qty(10), Some(Price(140)), 0); // price change -- loses queue priority
        let ExecReport::Resting { venue_order_id: id_after_modify, handle: handle_after_modify, .. } = reports[0] else { panic!("expected Resting, got {:?}", reports[0]) };
        assert_eq!(id_after_modify, original_id, "venue_order_id must survive a price-changing modify, matching real MCX's OrderID (\"remains constant\")");
        // sim_id (priority_ts here), by contrast, is the *other* identity
        // -- it deliberately does change, since queue priority genuinely
        // reset.
        assert_ne!(handle_after_modify.priority_ts, original_handle.priority_ts, "sim_id should still reset on a price-changing modify -- unrelated to venue_order_id");
        // The same id keeps working for a subsequent cancel, proving it's
        // not just echoed back unchanged but genuinely still the live key.
        let reports = ex.cancel(id_after_modify, 0);
        assert!(matches!(reports[0], ExecReport::Canceled { .. }));
    }
    ...
    fn otr_governor_also_gates_modify_not_just_new_orders() {
        ...
        let ExecReport::Resting { venue_order_id, .. } = reports[0] else { panic!("expected Resting, got {:?}", reports[0]) };
        let reports = ex.modify(venue_order_id, Qty(20), Some(Price(101)), 0);
        assert!(matches!(reports[0], ExecReport::Rejected { reason: RejectReason::OtrOrRateExceeded, .. }));
    }
