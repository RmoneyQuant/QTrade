...
impl ControlDispatcher {
    ...
    pub fn dispatch(&mut self, cache: &Cache, engine: &mut ExecutionEngine, outcome: &ExecOutcome) {
        let Some(handler) = &self.handler else { return };
        for fill in &outcome.fills {
            let mut ctx = Ctx::new(cache, engine, fill.timestamp_ns, DEFAULT_STRATEGY_ID, false);
            handler.borrow_mut().on_fill(&mut ctx, fill);
        }
        for event in &outcome.order_events {
            let mut ctx = Ctx::new(cache, engine, event.timestamp_ns, DEFAULT_STRATEGY_ID, false);
...
mod tests {
    ...
    fn subscribe_forwards_to_event_dispatcher_and_a_real_event_reaches_the_handler() {
        ...
        cache.apply(&event);
        event_dispatcher.on_book_touched(cache.book(instrument).unwrap(), instrument, &cache, &mut eng, 1, 0);
    ...
    fn engine(instruments: Vec<crate::types::Instrument>) -> ExecutionEngine {
    ...
    fn a_real_fill_reaches_on_fill_through_control_dispatcher() {
        ...
        let outcome = eng.on_market_event(&trade, 10);
        assert_eq!(outcome.fills.len(), 1, "the fill must actually be in what on_market_event returned");

        control_dispatcher.dispatch(&cache, &mut eng, &outcome);
    ...
    fn a_strategy_submitted_order_is_filled_and_delivered_through_on_fill() {
        ...
        // 1. A real book-touching event fires on_book, which submits.
        ...
        let submit_outcome = event_dispatcher.on_book_touched(cache.book(instrument).unwrap(), instrument, &cache, &mut eng, 1, 0);
        control_dispatcher.dispatch(&cache, &mut eng, &submit_outcome);
        let client_order_id = submitted.borrow().expect("on_book must have submitted");
        // "submit: gates passed" + "resting" -- both real order-events a
        // resting LimitDay submit produces, both already delivered.
        assert_eq!(order_updates.borrow().len(), 2, "the submit's own order-events must already have reached on_order_update");

        // 2. A real trade fully fills the resting Sell order.
        let trade = DecodedMessage::Trade(Trade { seq: 0, full: true, security_id: native_id, aggressor_side: DSide::Sell, price: DPrice(150), qty: DQty(intent_raw_qty()), event_time: 999 });
        let fill_outcome = eng.on_market_event(&trade, 10);
        control_dispatcher.dispatch(&cache, &mut eng, &fill_outcome);
