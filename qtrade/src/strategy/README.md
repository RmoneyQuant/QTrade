...
**Since the 2026-08-27 dual-clock pass, `submit`/`cancel`/`modify` are genuinely two-phase, not a same-call illusion.** Each one runs its local gate check synchronously (returning `Ok`/`Err` the same instant, as before) but the venue-reaching half is now a real event scheduled on `scheduler::Scheduler`, `latency_ns` later — `Ctx` carries a `RunHandles<'a> { venue, scheduler, pending_ops, latency_ns }` bundle (constructed by `main.rs`, one per callback) precisely so it has something to schedule onto. A strategy's own code doesn't change to use this — `ctx.submit(...)` still just returns `Ok(client_order_id)` — but a fill/reject notification on `on_fill`/`on_order_update` now genuinely arrives `latency_ns` after the call that caused it, not in the same instant. See `execution_user_doc.md` §2 and `main_user_doc.md` §3 item 6 for the mechanics.
