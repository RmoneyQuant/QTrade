...
pub struct Order {
    ...
    /// Real MCX's `OrderID` (tag 37) -- "generated by the MCX System; it
    /// remains constant." `None` until the venue's first `Resting`/
    /// `Filled` report for this order (never assigned to a `Denied`/
    /// `Rejected` order, which never reached the book), `Some` forever
    /// after -- including across a price-changing modify, unlike
    /// `simulator`'s own unrelated `sim_id` priority key. This, not
    /// `client_order_id`, is what real MCX Cancel/Replace requests
    /// require (§4.6/4.7) -- see `deliver_cancel_to_venue_inner`/
    /// `deliver_modify_to_venue_inner`.
...
impl ExecutionEngine {
    ...
    fn deliver_cancel_to_venue_inner(&mut self, client_order_id: u64, now_ns: u64, venue: &mut SimExchange) {
        // Real MCX Cancel requires `OrderID` (tag 37), not `ClOrdID`
        // (§4.6/4.7) -- `SimExchange::cancel` takes `venue_order_id`, not
        // `client_order_id`. `None` here means the order never reached
        // the venue at all (a `Denied` order), or a race already resolved
        // it terminally before this scheduled delivery fired -- either
        // way, there is nothing at the venue to cancel, same effective
        // outcome as `venue.cancel` finding nothing resting, just decided
        // one step earlier.
        let Some(venue_order_id) = self.orders.get(&client_order_id).and_then(|o| o.venue_order_id) else { return };
        self.venue_cancel_calls += 1;
        let reports = venue.cancel(venue_order_id, now_ns);
        self.handle_exec_reports(reports, now_ns);
    }
    ...
    fn deliver_modify_to_venue_inner(&mut self, client_order_id: u64, new_qty: Qty, new_price: Option<Price>, now_ns: u64, venue: &mut SimExchange) {
        // Same reasoning as `deliver_cancel_to_venue_inner`: real MCX
        // Replace also requires `OrderID`, not `ClOrdID` (§4.6/4.7).
        let Some(venue_order_id) = self.orders.get(&client_order_id).and_then(|o| o.venue_order_id) else { return };
        self.venue_modify_calls += 1;
        let reports = venue.modify(venue_order_id, new_qty, new_price, now_ns);
        self.handle_exec_reports(reports, now_ns);
    }
            ...
            match report {
                ...
                ExecReport::Resting { client_order_id, venue_order_id, handle, qty } => {
                    ...
                    if let Some(order) = self.orders.get_mut(&client_order_id) {
                        ...
                        // MCX's `OrderID`, tag 37), learned the instant
                        // the venue first mentions it, harmless to record
    ...
    fn on_fill(&mut self, client_order_id: u64, venue_order_id: u64, price: Price, qty: Qty, kind: FillKind, now_ns: u64) {
...
mod tests {
    ...
    #[test]
    fn a_denied_order_never_gets_a_venue_order_id_an_accepted_one_does() {
        // A Denied order never reaches the venue at all -- there is
        // nothing for SimExchange to assign real MCX's OrderID (tag 37)
        // to. Contrast with a real, accepted order, which does get one --
        // this is `ExecutionEngine`'s own record of what it would send if
        // this order were ever cancelled/modified against a real MCX.
        let (mut eng, mut venue) = engine(vec![future_instrument(1, 10, 100, 1)]);
        let denied_intent = NewOrderIntent { strategy_id: 1, instrument: IID, side: Side::Buy, order_type: OrderType::LimitDay(Price(105)), qty: Lots(5) };
        let (outcome, _) = submit_order_sync(&mut eng, &mut venue, denied_intent, 0);
        let GateOutcome::Denied { client_order_id: denied_id, .. } = outcome else { unreachable!() };
        assert_eq!(eng.order(denied_id).unwrap().venue_order_id, None);

        let accepted_intent = NewOrderIntent { strategy_id: 1, instrument: IID, side: Side::Buy, order_type: OrderType::LimitDay(Price(100)), qty: Lots(1) };
        let (outcome, _) = submit_order_sync(&mut eng, &mut venue, accepted_intent, 0);
        let GateOutcome::Submitted { client_order_id: accepted_id } = outcome else { unreachable!() };
        assert!(eng.order(accepted_id).unwrap().venue_order_id.is_some(), "an order the venue actually accepted must have a recorded venue_order_id");
    }
