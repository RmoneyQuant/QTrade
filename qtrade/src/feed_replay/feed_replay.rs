...
impl RecordSource {
    ...
    /// its contents. Returns `false` at a clean end-of-file.
...
/// One decoded message from the main increment replay, plus everything
/// about it a caller needs that isn't the message itself: a running
/// event counter, a synthetic monotonic clock, and the real exchange
/// feed-handler send time from the most recently seen `PacketHeader`.
///
/// `now_ns` is a synthetic, strictly-increasing counter, not a real
/// captured timestamp -- the per-message stream doesn't carry the outer
/// packet's real `TransactTime` at this granularity. Every consumer only
/// needs *a* monotonic clock, not the *real* one (unlike `scheduler`'s
/// `SimClock`, which this replay does not use -- no event loop, no
/// scheduling, just a straight-line replay).
///
/// `packet_transact_time_ns` is `PacketHeader.TransactTime` (template
/// 13003) -- the one per-message timestamp source this project has
/// verified is safe to use directly: some resting orders carry a
/// sentinel instead of a real time, and `Trade`'s own `event_time` field
/// isn't a timestamp at all (see `book_user_doc.md`'s `apply_trade`
/// finding).
pub struct ReplayEvent<'a> {
    pub event: &'a decoder::DecodedMessage,
    pub seq_no: u64,
    pub now_ns: u64,
    pub packet_transact_time_ns: u64,
}
...
pub fn replay(capture_path: &str, max_outer_records: u64, mut on_event: impl FnMut(ReplayEvent)) -> io::Result<ReplayStats> {
    ...
    let mut packet_transact_time_ns: u64 = 0;
    let mut payload = Vec::new();

    loop {
        match source.next_record(&mut payload) {
            Ok(true) => {}
            Ok(false) => break,
        ...
        while off + 8 <= payload.len() {
            ...
            if let decoder::DecodedMessage::PacketHeader(hdr) = &decoded {
                packet_transact_time_ns = hdr.transact_time;
            }
            now_ns += 1_000; // 1 microsecond per message, arbitrary but monotonic
            events += 1;

            on_event(ReplayEvent { event: &decoded, seq_no: events, now_ns, packet_transact_time_ns });
        }
